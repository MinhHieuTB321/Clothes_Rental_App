...
namespace OrderService.Application.Services
{
    public class Order_Service : IOrderService
    {
        ...
        public async Task<OrderReadModel> DeleteOrder(Guid id)
        {
            var order = await _unitOfWork.OrderRepository.GetByIdAsync(id) ?? throw new NotFoundException($"Order with ID-{id} is not exist!");
        ...
        {
            var order= await _unitOfWork.OrderRepository.GetByIdAsync(id,x=>x.Customer,x=>x.Shop!) ?? throw new NotFoundException($"Order with ID-{id} is not exist!");
        ...
        public async Task<OrderReadModel> UpdateOrder(OrderUpdateModel model)
        {
            var order = await _unitOfWork.OrderRepository.GetByIdAsync(model.Id) ?? throw new NotFoundException($"Order with ID-{model.Id} is not exist!");
