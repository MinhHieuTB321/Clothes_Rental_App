...
namespace ComboService.WebApi.Controllers
{
    public class ComboController : BaseController
    {
        ...
        /// </summary>
        [HttpGet]
        public async Task<ActionResult<IEnumerable<ComboResponseModel>>> GetAll(Guid? shopId)
        {
            var rs = await _service.GetCombos(shopId);
            return Ok(rs);
        }
        ...
        [HttpGet("{id}/products-combo")]
        public async Task<IActionResult> GetProductCombo(Guid id)
        {
            var rs = await _service.GetProductComboByComboId(id);
            return Ok(rs);
        }
        ...
        [HttpPost]
        public async Task<IActionResult> CreateCombo([FromForm] CreateComboRequestModel request)
        {
		...
		[Authorize(Roles = nameof(RoleEnum.Owner))]
        [HttpPut("{id}")]
        public async Task<IActionResult> UpdateCombo(Guid id, [FromForm] UpdateComboRequestModel request)
        {
            var rs = await _service.UpdateCombo(request);
			try
			{
