...
namespace ComboService.Application.Services
{
    public class CombosService : IComboService
    {
        ...
        public async Task<ComboResponseModel> CreateCombo(CreateComboRequestModel request)
        {
            ...
            if (checkCombo != null)
            {
                throw new Exception("Combo has already existed!!!");
            }
        ...
        public async Task<List<ProductComboResponseModel>> GetProductComboByComboId(Guid Id)
        {
            var productCombos = await _unitOfWork.Repository<ProductCombo>()
                .GetAll()
                .Where(x => x.ComboId.Equals(Id)&& x.IsDeleted==false)
                .Include(x=>x.Product)
        ...
        public async Task<IEnumerable<ComboResponseModel>> GetCombos(Guid? shopId)
        {
           var combos = await  _unitOfWork.Repository<Combo>()
                ...
                .ToListAsync();
            if(shopId!=null) combos= combos.Where(x=>x.ShopId==shopId).ToList();
        ...
        public async Task<ComboResponseModel> UpdateCombo(UpdateComboRequestModel request)
        {
            Combo combo = _unitOfWork.Repository<Combo>().Find(x => x.Id.Equals(request.Id)&&x.IsDeleted==false);
            if(combo == null)
            {
                throw new NotFoundException("Combo is not exist!");
            }
